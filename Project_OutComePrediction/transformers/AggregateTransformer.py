...
class AggregateTransformer(TransformerMixin):
    ...
    def transform(self, X, y=None):
        start = time()
        print("🧪 Columns in self.num_cols:", self.num_cols)
        print("🧪 Available columns in X:", list(X.columns))
        agg_dict = {col: ['mean', 'max', 'min', 'sum', 'std'] for col in self.num_cols}
        # transform numeric cols
        if len(self.num_cols) > 0:
            ddt_numeric = X.groupby(self.case_id_col)[self.num_cols].agg(agg_dict)
            dt_numeric.columns = ['{}_{}'.format(col, stat) for col, stat in dt_numeric.columns]
